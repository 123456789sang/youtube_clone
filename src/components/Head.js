    ...
    return (
        <div className={`fixed top-0 left-0 right-0 z-50 bg-white grid h-16 grid-flow-col py-3 px-2 shadow-sm ${showMobileSearch ? 'sm:grid' : 'grid'}`}>
            ...
            <div className='flex justify-between gap-2 sm:gap-3 md:gap-4 xl:gap-6 '>
                ...
                {/* searchbar  */}
                <div className="w-full sm:w-9/12   relative flex items-center justify-between">
                    {/* Desktop & tablet */}
                    <div className="hidden sm:flex justify-center items-center w-full">
                        <input
                            className="w-[70%] border border-gray-300 pl-4 p-2 rounded-l-full"
                            type="text"
                            placeholder="Search ...."
                            value={searchQuery}
                    ...
                    {showSuggestions && suggestion?.length > 0 &&(
                        <div className="absolute top-11 ml-8 sm:-ml-4 md:ml-4 xl:ml-16 bg-white py-2 px-2 w-[80%] sm:w-[25rem] md:w-[36rem] shadow-lg rounded-lg border border-gray-100">
                            <ul>
                            {suggestion.map((s) => (
                                <li
                                key={s}
